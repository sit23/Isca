...
! Modify to loop over all columns (nlon) or over daylight columns
      ...
      do iplon = 1, ncol
!mj do not run if there is no SW 
         if ( coszen(iplon) < zepzen ) then
            swuflx (iplon,:) = 0.
            swdflx (iplon,:) = 0.
            swhr   (iplon,:) = 0.
            swuflxc(iplon,:) = 0.
...
! 1) Use Earth/Sun distance flux adjustment provided by GCM (input as adjes);
      adjflx = adjes
!
! 2) Calculate Earth/Sun distance from DYOFYR, the cudyofyrmulative day of the year.
...
!         wkl(1,l) = h2ovmr(iplon,l)/(1._rb - h2ovmr(iplon,l))
         ...
         wkl(3,l) = o3vmr(iplon,l)*amdo !o3 input in mmr
