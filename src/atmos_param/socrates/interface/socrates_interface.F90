...
!   INTEGER :: id_soc_surf_spectrum_sw !not implemented yet
  ...
  INTEGER :: id_soc_flux_lw_up, id_soc_flux_lw_down
  ...
  INTEGER :: id_soc_surf_flux_sw_clr, id_soc_surf_flux_sw_down_clr, &
             id_soc_surf_flux_lw_clr, id_soc_surf_flux_lw_down_clr
  ...
  INTEGER :: id_mars_solar_long, id_rrsun, id_true_anom,  id_time_since_ae, id_dec, id_ang, id_t_half
  ...
  REAL(r_def), allocatable, dimension(:,:,:) :: thd_co2_store, thd_ozone_store, t_half_store
...
!         'watts/m2', missing_value=missing_value )
   ...
   id_soc_surf_flux_lw_down = &
    ...
    id_soc_olr_clr = &
        register_diag_field ( soc_mod_name, 'soc_olr_clr', axes(1:2), Time, &
        'clear-sky socrates TOA LW flux (up)', &
        'watts/m2', missing_value=missing_value )

    id_soc_toa_sw_clr = &
        register_diag_field ( soc_mod_name, 'soc_toa_sw_clr', axes(1:2), Time, &
    ...
    id_soc_surf_flux_lw_clr = &
        register_diag_field ( soc_mod_name, 'soc_surf_flux_lw_clr', axes(1:2), Time, &
        'clear-sky socrates Net LW surface flux (up)', &
        'watts/m2', missing_value=missing_value )

    id_soc_surf_flux_lw_down_clr = &
        register_diag_field ( soc_mod_name, 'soc_surf_flux_lw_down_clr', axes(1:2), Time, &
        'clear-sky socrates LW surface flux down', &
        'watts/m2', missing_value=missing_value )

    id_soc_surf_flux_sw_clr = &
        register_diag_field ( soc_mod_name, 'soc_surf_flux_sw_clr', axes(1:2), Time, &
        'clear-sky socrates Net SW surface flux (down)', &
        'watts/m2', missing_value=missing_value )

    id_soc_surf_flux_sw_down_clr = &
        register_diag_field ( soc_mod_name, 'soc_surf_flux_sw_down_clr', axes(1:2), Time, &
        'clear-sky socrates SW surface flux down', &
        'watts/m2', missing_value=missing_value )
    ...
    id_t_half = &
         register_diag_field ( soc_mod_name, 't_half_soc', (/axes(1),axes(2),axes(4)/), Time, &
         'temp on half levels', &
    ...
    if(store_intermediate_rad) then
        ...
        if (id_soc_surf_flux_sw_clr > 0) then
            allocate(net_surf_sw_down_clr_store(size(lonb,1)-1, size(latb,2)-1))
        endif

        if (id_soc_surf_flux_sw_down_clr > 0) then
            allocate(surf_sw_down_clr_store(size(lonb,1)-1, size(latb,2)-1))
        endif

        if (id_soc_surf_flux_lw_clr > 0) then
            allocate(surf_lw_net_clr_store(size(lonb,1)-1, size(latb,2)-1))
        endif

        if (id_soc_surf_flux_lw_down_clr > 0) then
            allocate(surf_lw_down_clr_store(size(lonb,1)-1, size(latb,2)-1))
        endif
        ...
        if (id_t_half > 0 ) then
            allocate(t_half_store(size(lonb,1)-1, size(latb,2)-1, num_levels+1))
        endif        
  ...
  subroutine socrates_interface(Time_diag, rlat, rlon, soc_lw_mode,  &
    ...
    real(r_def), intent(out) :: output_flux_up_clr(:,:,:)
    real(r_def), intent(out) :: output_flux_down_clr(:,:,:)
...
!              input_t_level(:,0) = input_t(:,1) - (input_t_level(:,1) - input_t(:,1))
    ...
    ! write(6,*) 'dmass', input_d_mass(1,:)*grav, size(input_d_mass,1), size(input_d_mass,2)
    ! write(6,*) 'input_p_level', input_p_level(1,:), size(input_p_level,1), size(input_p_level,2)
    ! write(6,*) 'fms_p_half', fms_p_half(1, 1,:), size(fms_p_half,1), size(fms_p_half,2), size(fms_p_half,3)
    ...
    DO i_chunk=1,n_chunk_loop
        ...
        if (soc_lw_mode .eqv. .TRUE.) then
            ...
            ! write(6,*) 'pressures', input_p(idx_chunk_start:idx_chunk_end,:)
            ! write(6,*) 'half pressures', input_p_level(idx_chunk_start:idx_chunk_end,:)
            ! write(6,*) 't', input_t(idx_chunk_start:idx_chunk_end,:)           
            ! write(6,*) 'tl', input_t_level(idx_chunk_start:idx_chunk_end,:)   
            ! write(6,*) 'co2', input_co2_mixing_ratio(idx_chunk_start:idx_chunk_end,:)   
            ! write(6,*) 'mix', input_mixing_ratio(idx_chunk_start:idx_chunk_end,:)               
            ! write(6,*) 'ts', input_t_surf(idx_chunk_start:idx_chunk_end)               
    ...
    output_flux_up_clr(:,:,:) = reshape(soc_flux_up_clr(:,:),(/si,sj,sk+1 /))
    output_flux_down_clr(:,:,:) = reshape(soc_flux_down_clr(:,:),(/si,sj,sk+1 /))

    if(present(output_flux_direct)) then
        output_flux_direct(:,:,:) = reshape(soc_flux_direct(:,:),(/si,sj,sk+1 /))
    endif

    if(present(output_flux_direct_clr)) then
        output_flux_direct_clr(:,:,:) = reshape(soc_flux_direct_clr(:,:),(/si,sj,sk+1 /))
    endif
...
subroutine run_socrates(Time, Time_diag, rad_lat, rad_lon, temp_in, q_in, t_surf_in, p_full_in, p_half_in, z_full_in, z_half_in, albedo_in, &
    ...
    real(r_def), dimension(size(temp_in,1), &
        size(temp_in,2), size(temp_in,3)) :: tg_tmp_soc, q_soc, ozone_soc, co2_soc, p_full_soc, &
    ...
    real(r_def), dimension(size(temp_in,1), &
        size(temp_in,2), size(temp_in,3)+1) :: p_half_soc, t_half_out, z_half_soc, output_soc_flux_sw_down, &
                                            ...
                                            t_half_out_clr
    real(r_def), dimension(size(temp_in,1), size(temp_in,2)) :: tot_cloud_cover
    ...
    real, dimension(size(temp_in,1), size(temp_in,2)) :: coszen, fracsun, surf_lw_net, olr, toa_sw, &
                                                         ...
                                                         net_surf_sw_down_clr, surf_sw_down_clr, surf_lw_net_clr, surf_lw_down_clr
    real, dimension(size(temp_in,1), size(temp_in,2), size(temp_in,3)) :: ozone_in, co2_in
    real, dimension(size(temp_in,1), size(temp_in,2), size(temp_in,3)+1) :: thd_sw_flux_net, thd_lw_flux_net, thd_sw_flux_clr_net, thd_lw_flux_clr_net, thd_lw_flux_up, thd_lw_flux_down
    ...
    if(r_total_seconds - dt_last .ge. dt_rad) then
        ...
        if(store_intermediate_rad) then
            ...
            if (id_soc_surf_flux_sw_clr > 0) then
                net_surf_sw_down_clr = net_surf_sw_down_clr_store
            endif

            if (id_soc_surf_flux_sw_down_clr > 0) then
                surf_sw_down_clr = surf_sw_down_clr_store
            endif

            if (id_soc_surf_flux_lw_clr > 0) then
                surf_lw_net_clr = surf_lw_net_clr_store
            endif

            if (id_soc_surf_flux_lw_down_clr > 0) then
                surf_lw_down_clr = surf_lw_down_clr_store
            endif
        ...
        if(id_soc_olr_clr > 0) then
            used = send_data ( id_soc_olr_clr, olr_clr, Time_diag)
        endif
        if(id_soc_toa_sw_clr > 0) then
        ...
        if (id_soc_surf_flux_sw_clr > 0) then
            used = send_data ( id_soc_surf_flux_sw_clr, net_surf_sw_down_clr, Time_diag)
        endif
        if (id_soc_surf_flux_sw_down_clr > 0) then
            used = send_data ( id_soc_surf_flux_sw_down_clr, surf_sw_down_clr, Time_diag)
        endif
        if (id_soc_surf_flux_lw_clr > 0) then
            used = send_data ( id_soc_surf_flux_lw_clr, surf_lw_net_clr, Time_diag)
        endif
        if (id_soc_surf_flux_lw_down_clr > 0) then
            used = send_data ( id_soc_surf_flux_lw_down_clr, surf_lw_down_clr, Time_diag)
        endif
        ...
        if (id_t_half > 0) then
            used = send_data ( id_t_half, t_half_out, Time_diag)        
        endif            
        ! Diagnostics sent 
    ...
    surf_lw_down_clr(:,:) = REAL(output_soc_flux_lw_down_clr(:,:, n_layer+1))
    surf_lw_net_clr(:,:) = REAL(output_soc_flux_lw_up_clr(:,:,n_layer+1) - output_soc_flux_lw_down_clr(:,:, n_layer+1))
    ...
    net_surf_sw_down_clr(:,:) = REAL(output_soc_flux_sw_down_clr(:,:, n_layer+1)-output_soc_flux_sw_up_clr(:,:,n_layer+1) )
    surf_sw_down_clr(:,:) = REAL(output_soc_flux_sw_down_clr(:,:, n_layer+1))
    ...
    if(store_intermediate_rad)then
        ...
        if (id_soc_surf_flux_sw_clr > 0) then
            net_surf_sw_down_clr_store = net_surf_sw_down_clr
        endif

        if (id_soc_surf_flux_sw_down_clr > 0) then
            surf_sw_down_clr_store = surf_sw_down_clr
        endif

        if (id_soc_surf_flux_lw_clr > 0) then
            surf_lw_net_clr_store = surf_lw_net_clr
        endif

        if (id_soc_surf_flux_lw_down_clr > 0) then
            surf_lw_down_clr_store = surf_lw_down_clr
        endif
